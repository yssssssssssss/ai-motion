...
function generateOrSave(e) {
  if (e.metaKey && window.domtoimage) {
    saveGradient(e);
  } else {
    generateGradient();
  }
...
window.motionReplay = function motionReplay() {
  document.documentElement.style.setProperty("--motion-play-state", "running");
  document.body.classList.add("is-resizing");
  restartAnimation(root);
  restartAnimation(document.querySelector(".phone"));
  setTimeout(() => document.body.classList.remove("is-resizing"), delay);
};

window.motionPause = function motionPause() {
  document.documentElement.style.setProperty("--motion-play-state", "paused");
};
