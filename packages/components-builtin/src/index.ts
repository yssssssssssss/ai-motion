...
type GlobOptions = { query?: string; import?: string; eager?: true };
declare global {
  interface ImportMeta {
    glob: (pattern: GlobPattern, options?: GlobOptions) => Record<string, string>;
  }
}

// 把每个组件目录下的所有文件按相对路径作为原始字符串注入。
// 路径键形如 '../<component-id>/<relative-path>'
const rawFiles = import.meta.glob(["../*/**/*", "!../src/**"], {
  query: "?raw",
  import: "default",
  eager: true
});
...
function groupByComponent(): Record<string, Record<string, string>> {
  ...
  for (const [absolutePath, content] of Object.entries(rawFiles)) {
